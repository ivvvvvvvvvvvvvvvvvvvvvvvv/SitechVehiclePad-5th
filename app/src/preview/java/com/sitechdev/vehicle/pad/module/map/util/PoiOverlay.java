...
public class PoiOverlay {
     ...
     * @since V2.1.0
     */
    public PoiOverlay(Context context, AMap amap, List<PoiItem> pois) {
        mContext = context;
        mAMap = amap;
        mPois = pois;
    }
    ...
    public void addToMap() {
        try {
            for (int i = 0; i < mPois.size(); i++) {
                ...
                //设置动画
                marker.setAnimation(MapAnimUtil.getGrowAnimation());
                //开始动画
        ...
        try {
            if (mPois != null && mPois.size() > 0) {
                if (mAMap == null)
                    return;
                if (mPois.size() == 1) {
                    mAMap.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(mPois.get(0).getLatLonPoint().getLatitude(),
                            mPois.get(0).getLatLonPoint().getLongitude()), 18f));
                } else {
                    LatLngBounds bounds = getLatLngBounds();
                    mAMap.animateCamera(CameraUpdateFactory.newLatLngBounds(bounds, 300));
                }
            }
        } catch (Throwable e) {
            e.printStackTrace();
    ...
    protected BitmapDescriptor getBitmapDescriptor(int index) {
        View view = View.inflate(mContext, R.layout.custom_poi_single, null);
        TextView tvTitle = view.findViewById(R.id.id_tv_title);
        TextView tvDesc = view.findViewById(R.id.id_tv_desc);
        TextView tvIndex = view.findViewById(R.id.id_poi_index);
        tvTitle.setText(getTitle(index));
        tvDesc.setText(getSnippet(index));
