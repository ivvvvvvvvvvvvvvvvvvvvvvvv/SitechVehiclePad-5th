...
public class FeedBackUtils extends HttpUtil {
                        ...
                        switch (feedBackTypeBean.getCode()) {
                            case AppUrlConst.HTTP_CODE_200:
                                if (feedBackTypeBean.getData() != null && feedBackTypeBean.getData().getTypeList() != null) {
                                    List<FeedBackTypeBean.BackTypeBean.TypeBean> typeBeanList =
                                            feedBackTypeBean.getData().getTypeList();
                                    for (int i = 0; i < typeBeanList.size(); i++) {
                                        FeedBackTypeBean.BackTypeBean.TypeBean typeBean =
                                                typeBeanList.get(i);
     ...
     * @param baseBribery
     */
    public static void commitFeedBack(String feedbackType, String content, List<String> imgFile,
                                      BaseBribery baseBribery) {
    ...
    public static void commitFeedBackUploadImg(ArrayList<File> imgFiles, BaseBribery baseBribery) {
        ...
        if (imgFiles != null && !imgFiles.isEmpty()) {
            for (File file : imgFiles) {
                HttpParams.FileWrapper fileWrapper = new HttpParams.FileWrapper(file,
                        file.getName(), MediaType.parse("image/*"));
        ...
        OkGo.<String>post(formatBBSUrlRequestUrl(AppUrlConst.URL_FEEDBACK_IMAGE_UPLOAD))
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        SitechDevLog.i(AppConst.TAG, "OkGo--callback---onSuccess====response" +
                                "========>" + response);
                    }
                    ...
                    public void onError(Response<String> response) {
                        ...
                        SitechDevLog.i(AppConst.TAG, "OkGo--callback---onError====response" +
                                "========>" + response);
                    }
                    ...
                    public String convertResponse(okhttp3.Response response) throws Throwable {
                        if (null != response && response.isSuccessful()) {
                            try {
                                ...
                                SitechDevLog.i(AppConst.TAG, "OkGo--callback---onSuccess" +
                                    ...
                                    case AppUrlConst.HTTP_CODE_200:
                                        if (bodyObject.has("data") && !StringUtils.isEmpty(bodyObject.optString("data"))) {
                                            List<String> imgUrlList =
                                                    GsonUtils.jsonToList(bodyObject.optString(
                                                            "data"), String.class);
                                            if (baseBribery != null) {
    ...
    private static String getErrorMsg(String errorMsg) {
        return StringUtils.isEmpty(errorMsg) ? "数据请求失败，请稍后再试！" : errorMsg;
    }

    public interface OnLoadFeedbackHistoryCallBack<T> {
